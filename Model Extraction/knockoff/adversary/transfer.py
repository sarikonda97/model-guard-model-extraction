...
class AdaptiveAdversary(object):
    ...
    def get_transferset(self, budget):
        ...
        dummy_set = self.idx_set

        blacklist_class = []

        ##### Find the class with less than prob
        visited = []
        for index in dummy_set:
            if self.queryset[index][1] not in visited:
                x_t = torch.stack([self.queryset[index][0]]).to(self.blackbox.device)
                y_t = self.blackbox(x_t).cpu()
                # print("The output vector is:", y_t.numpy()[0])
                # print("The max is", max(y_t.numpy()[0]))
                if max(y_t.numpy()[0]) < 0.6:
                    blacklist_class.append(self.queryset[index][1])
                visited.append(self.queryset[index][1])
            ## This is valid for CIFAR100
            if len(visited)==100:
                break
        
        # print("The no of visited classes:", len(visited))
        print("The no of blacklisted classes are:", len(blacklist_class))
        #### FIlter the classes based on scores

        selected_indices = []
        removed_indices = []
        for index in dummy_set:
            if self.queryset[index][1] not in blacklist_class:
                selected_indices.append(index)
            else:
                removed_indices.append(index)

        # print("No of indices selected: ", len(selected_indices))
        # print("No of removed indices are: ", len(removed_indices))

        if len(selected_indices) < budget:
            selected_indices.extend(random.sample(removed_indices, budget-len(selected_indices)))
        
        # print("After transforming :", len(selected_indices))

        self.idx_set = set(selected_indices)

        print("The len of idx_set is:", len(self.idx_set))
...
def main():
    ...
    if queryset_name == 'ImageFolder':
        ...
        queryset = datasets.__dict__[queryset_name](root=params['root'], transform=transform, img_tcq=img_obfs_tcq, img_mag=img_obfs_mag)
    else:
        queryset = datasets.__dict__[queryset_name](train=True, transform=transform, img_tcq=img_obfs_tcq, img_mag=img_obfs_mag)
