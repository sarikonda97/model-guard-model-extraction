...
for blur_effect in blur:
    subprocess.call(["python", "knockoff/adversary/train.py", f"models/adversary/cifar10-alexnet-random-blur{blur_effect}-{budget}", "resnet34", "CIFAR10", "--budgets", f"{budget}", "-d", "0", "--pretrained", "imagenet", "--img_obfs_tcq", "blur", "--img_obfs_mag", f"{blur_effect}", "--log-interval", "100", "--epochs", f"{epochs}", "--lr", "0.01"])
